...
// led
const unsigned char ledGreenPin = 4;      // the Green LED pin
const unsigned char ledRedPin = 3;        // the Red LED pin
...
static int SimpleXmlParser(char* payload, char* name, char* value, int isPC) {
    ...
    if(s && e) {
        memcpy(value, s, e-s);
		Stream_print(NULL, "[%s:%s]", name, value);
    }    
    return rc;
}
...
static void ProcessCMD(char* payload, int payloadLen) {
	...
	if(strcmp(cmt, CMT_REPPERCHANGE) == 0) {
		...
		} else if(strcmp(value, "R0") == 0) {
			digitalWrite(ledRedPin, LOW);
		} else if(strcmp(value, "G1") == 0) {
...
unsigned int mReportInterval = 5000; // 5sec
int mLCDStatus = -1;
/**
 * @brief main
 * @param[in] argc
...
int MARun() {
    ...
    while (mStep < PROCESS_END) {
		...
		int dist = SMAGetDistance();
		Stream_print(NULL,"distance : %d\n", dist);
		
		if(dist < 30 && mLCDStatus != 1) {
			mLCDStatus = 1;
			lcd.clear();
			lcd.setCursor(0, 0);
			lcd.print("Welcome to");
			lcd.setCursor(0, 1);
			lcd.print(" LoRa's House!");
			lcd.setRGB(255, 255, 255);
		} else if(dist >= 30 && mLCDStatus != 0) {
            mLCDStatus = 0;
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print("powered by");
            lcd.setCursor(0, 1);
            lcd.print("SKT ThingPlug");
            lcd.setRGB(0, 255, 0);          
        }

		if(mStep == PROCESS_CONTENTINSTANCE_CREATE && (currentMillis - oldMillis) > mReportInterval ) {
