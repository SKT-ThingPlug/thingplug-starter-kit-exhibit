...
//-------------------------------- 1. Container에 저장된 사진 값 조회(Retrieve)----------------------------------------//
function getLatestPhotoURL(cb){
httpReq({ 
  options: {
    host : config[configIndex].TPhost,
    port : config[configIndex].TPport,
    path : '/'+config[configIndex].AppEUI+'/'+config[configIndex].version+'/remoteCSE-'+ config[configIndex].RaspID+ '/container-'+ config[configIndex].RaspID+'_photoURL'+'/latest',
    method: 'GET',
    headers : {
      Accept: 'application/json',
      locale: 'ko',
      uKey : config[configIndex].uKey,
      'X-M2M-RI': config[configIndex].RaspID+'_'+randomInt(100000, 999999),
      'X-M2M-Origin': config[configIndex].RaspID
    }
  }
}).then(function(result){
  if(result.data){
